...
from dataset import load_celeb_a_dataset  # Import the dataset loading function

# batch size, epochs (time), CUDA (helper)/GPU
...
# sets up a random seed for reproducibility while training

torch.manual_seed(args.seed)
if args.cuda:
    torch.cuda.manual_seed(args.seed)

# loads datasets using load_celeb_a_dataset from dataset.py
print('===> Loading datasets')
ds = load_celeb_a_dataset(split='train')
images_A = np.array([example['image'].numpy() / 255.0 for example in tfds.as_numpy(ds)])
images_B = images_A.copy()
images_A += images_B.mean(axis=(0, 1, 2)) - images_A.mean(axis=(0, 1, 2))

model = Autoencoder().to(device)

# checks for saved model checkpoints
# decides whether to load checkpoint or start training from beginning
